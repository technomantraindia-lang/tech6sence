...
export default function ExpertWorkshops() {
  const blocks = [
    {
      ...
      icon: (
        <svg className="w-8 h-8 text-violet-500" fill="currentColor" viewBox="0 0 576 512">
    ...
    },
    {
      ...
      icon: (
        <svg className="w-8 h-8 text-violet-500" fill="currentColor" viewBox="0 0 448 512">
  ...
  return (
    <section className="relative bg-white text-slate-900 py-20 md:py-28 overflow-hidden">
      <div className="max-w-[85rem] mx-auto px-6 relative z-10 grid grid-cols-1 md:grid-cols-2 gap-8 lg:gap-12">
        
        {blocks.map((block, idx) => (
          <div key={idx} className="bg-slate-50 border border-slate-200 rounded-[2.5rem] p-10 md:p-14 hover:shadow-lg transition-shadow duration-300 group flex flex-col justify-between">
            <div>
