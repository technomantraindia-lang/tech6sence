      ...
      <div className="max-w-[1400px] mx-auto px-6 relative z-10">
        <div className="text-center max-w-3xl mx-auto mb-16">
          <div className="flex items-center justify-center gap-2 mb-4">
            ...
            <span className="font-mono text-xs font-bold text-cyan-600 uppercase tracking-widest" style={{ fontFamily: "'Plus Jakarta Sans', sans-serif" }}>
              Membership Tiers
            </span>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {tiers.map((tier, idx) => (
            <div key={idx} className="bg-white rounded-3xl p-8 border border-slate-200 shadow-sm flex flex-col hover:-translate-y-1 hover:shadow-md transition-all duration-300">
              <h3 className="text-2xl font-bold text-slate-900 mb-2" style={{ fontFamily: "'Space Grotesk', sans-serif" }}>{tier.name}</h3>
              <p className="text-sm text-slate-500 mb-6 flex-grow" style={{ fontFamily: "'Plus Jakarta Sans', sans-serif" }}>{tier.desc}</p>
              ...
              <ul className="space-y-4 mb-8 border-t border-slate-100 pt-6">
                {tier.features.map((feature, i) => (
                  <li key={i} className="flex items-start gap-3">
                    ...
                    <span className="text-sm text-slate-700 font-medium" style={{ fontFamily: "'Plus Jakarta Sans', sans-serif" }}>{feature}</span>
                  </li>
                ))}
              </ul>
              
              <div className="mt-auto">
                <Link to="#business-brain-commu" className="block w-full text-center py-3 px-4 rounded-xl font-bold text-sm bg-slate-50 border border-slate-200 hover:bg-slate-100 text-slate-900 transition-colors" style={{ fontFamily: "'Plus Jakarta Sans', sans-serif" }}>
                  Get Started
                </Link>
              </div>
            </div>
