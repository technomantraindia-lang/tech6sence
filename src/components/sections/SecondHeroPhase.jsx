...
const RealisticGlobeCanvas = lazy(() => import('../hero/RealisticGlobe'))
...
const getConnectorPaths = (width, height, progress, cardsReveal, overScroll = 0) => {
  ...
  // Center Y of the globe matches the calculation in Home.jsx exactly!
  const startSize = Math.min(width * 1.53, 1785)
  const startCenterY = -100 - height + startSize / 2
  const endCenterY = height * 0.44
  const ringCenterY = progress < 1
    ? startCenterY + (endCenterY - startCenterY) * progress
    : endCenterY + 2 * overScroll
...
export default function SecondHeroPhase({ scrollY }) {
  ...
  // Calculate local progress and overScroll dynamically based on scrollY prop
  let progress = 0
  let overScroll = 0
  if (scrollRef.current && typeof window !== 'undefined') {
    ...
    if (scrollY >= elementTop) {
      ...
      if (scrollY - elementTop > scrollable) {
        overScroll = scrollY - elementTop - scrollable
      }
    }
  ...
  const cardsReveal = progress < 0.3 ? 0 : (progress - 0.3) / 0.7
  const translateY = progress * 14

  return (
    <section ref={scrollRef} className="relative h-[190vh] bg-[#03060f]">
      <div className="sticky top-0 h-screen w-full overflow-hidden">
        {/* Base Background */}
        <div className="absolute inset-0 z-0 bg-[#03060f]">
          <div className="absolute inset-0" style={{ background: 'radial-gradient(circle at 50% 50%, #1d0742 0%, #0d0421 55%, #03060f 100%)' }} />
          {/* Extended super-soft blend overlay to transition from TechHero background (#03060f) */}
          <div className="absolute top-0 inset-x-0 h-[400px] bg-gradient-to-b from-[#03060f] via-[#03060f]/60 to-transparent pointer-events-none z-[1]" />
        </div>
        ...
        {/* Network Globe Container - Handled at page level by Home.jsx */}
        ...
        <svg className="pointer-events-none absolute inset-0 z-[24] h-full w-full hidden lg:block">
          ...
          {getConnectorPaths(winSize.w, winSize.h, progress, cardsReveal, overScroll).map((path, idx) => (
