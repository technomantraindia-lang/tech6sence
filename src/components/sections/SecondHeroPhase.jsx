...
const getConnectorPaths = (width, height, progress, cardsReveal) => {
  ...
  const ringRadius = (globeSize / 2) * 0.875
...
export default function SecondHeroPhase() {
  ...
  // Smooth reveal maps progress directly to cards reveal
  const cardsReveal = progress
  
  // Parallax displacement down for globe
  const translateY = progress * 14

  return (
    <section ref={scrollRef} className="relative h-[220vh] bg-[#0a031d] border-t border-violet-500/20">
      <div className="sticky top-0 h-screen w-full overflow-hidden">
        ...
        {/* Top pill */}
        <div className="pointer-events-none absolute inset-x-0 top-[10vh] z-[6] flex justify-center px-4"
        ...
        >
          <div 
            className="absolute left-1/2 w-full max-w-[min(92vw,450px)]"
            style={{
              top: '50%',
        ...
        <svg className="pointer-events-none absolute inset-0 z-[24] h-full w-full hidden lg:block">
          ...
          {getConnectorPaths(winSize.w, winSize.h, progress, cardsReveal).map((path, idx) => (
            <g key={path.id} style={{ opacity: path.reveal, transition: 'opacity 0.6s cubic-bezier(0.16, 1, 0.3, 1)' }}>
        ...
        {/* Mobile cards grid */}
        <div className="pointer-events-none absolute inset-x-3 bottom-10 z-[25] grid grid-cols-2 gap-2.5 lg:hidden">
        ...
        <div className="pointer-events-none absolute inset-x-0 bottom-10 z-[30] hidden text-center md:block"
             style={{ opacity: progress, transform: `translateY(${(1 - progress) * 16}px)` }}
        >
