...
const INDUSTRIES = [
  {
    ...
    nodes: [
      ...
      { cx: 112, cy: 144, label: 'Patient AI' },
...
const IndustryVisual = memo(({ industry }) => {
  ...
  return (
    <svg viewBox="0 0 224 170" className="w-full h-full">
      ...
      {/* Center hub */}
      <circle cx={center.cx} cy={center.cy} r="16" fill={industry.accent} opacity="0.35" className="ind-pulse" />
...
IndustryVisual.displayName = 'IndustryVisual';
...
export default function IndustriesTransform() {
  ...
  // IntersectionObserver for tracking active card in view
  useEffect(() => {
    if (!isVisible) return;

    const observerOptions = {
      root: null,
      rootMargin: '-35% 0px -40% 0px', // focused center threshold
      threshold: 0.1,
    };
  ...
  return (
    <section
      ref={sectionRef}
      id="industries"
      className="relative w-full overflow-visible bg-gradient-to-b from-[#FAFAFA] via-[#F7F6FF] to-[#FAFAFA] py-16 md:py-28"
    >
      ...
      {/* Grid Container */}
      <div
        className="mx-auto max-w-[85rem] px-6 transition-all duration-1000 ease-out relative"
        style={{
          opacity: isVisible ? 1 : 0,
          transform: isVisible ? 'translateY(0)' : 'translateY(30px)',
        }}
      >
        {/* Two-column sticky interactive layout */}
        <div className="grid md:grid-cols-[1.1fr_1.4fr] gap-8 md:gap-12 lg:gap-16 items-start">
          
          {/* LEFT COLUMN: Sticky heading & dynamic active details info card */}
          <div className="industries-left-sticky flex flex-col gap-6">
            <div className="max-w-xl">
              ...
              <p className="font-body text-[0.92rem] leading-[1.7] text-slate-500 font-medium mb-1">
                TECH6SENSE AI powers businesses, high-growth startups, and legacy enterprises by introducing bespoke, secure, and production-ready artificial intelligence ecosystems.
              </p>
            </div>

            {/* Dynamic Active Industry Detail Card */}
            <div 
              key={activeIndustry.id}
              className="industry-detail-transition relative overflow-hidden rounded-2xl border bg-white/80 backdrop-blur-md p-6 md:p-7 shadow-[0_12px_40px_rgba(0,0,0,0.02)]"
              style={{ borderColor: `${activeIndustry.accent}20` }}
            >
              <div 
                className="absolute top-0 right-0 w-28 h-28 rounded-bl-full pointer-events-none transition-all duration-500" 
                style={{
                  background: `linear-gradient(225deg, ${activeIndustry.accent}10, transparent)`,
                }}
              />
              ...
              <div className="flex items-center gap-3.5 mb-4">
                <div
                  ...
                  style={{
                    background: activeIndustry.accent + '15',
                    borderColor: activeIndustry.accent + '35',
                ...
                <div>
                  <h4 className="font-display text-[1.15rem] font-extrabold text-slate-900 tracking-tight">
                    {activeIndustry.name}
                  </h4>
                  <p className="font-body text-[0.76rem] font-semibold tracking-wide text-slate-400 mt-0.5" style={{ color: activeIndustry.accent }}>
                    {activeIndustry.tagline}
                  </p>
                </div>
              </div>

              <p className="font-body text-[0.88rem] leading-relaxed text-slate-600 mb-6">
                ...
                <div className="flex flex-col gap-2.5">
                  {activeIndustry.applications.map((app, i) => (
                    <div key={i} className="flex items-center gap-3">
                      <div
                        className="w-5 h-5 shrink-0 rounded-md flex items-center justify-center transition-colors duration-300"
                        style={{ background: activeIndustry.accent + '15' }}
                      >
                        <svg viewBox="0 0 16 16" className="w-2.5 h-2.5" fill="none" stroke={activeIndustry.accent} strokeWidth="3">
                          <path strokeLinecap="round" strokeLinejoin="round" d="M3 8.5l3.5 3.5L13 4" />
                        </svg>
                      </div>
                      <span className="font-body text-[0.82rem] text-slate-600 font-semibold">{app}</span>
                    </div>
                  ))}
                </div>
              </div>
          ...
          {/* RIGHT COLUMN: Scrolling Stack of Capability Cards (Simplified) */}
          <div className="industries-right-stack flex flex-col gap-6">
            {INDUSTRIES.map((ind) => {
              ...
              return (
                <div
                  ...
                  className={`industry-capability-card relative rounded-3xl border p-5 md:p-7 shadow-[0_15px_45px_rgba(0,0,0,0.01)] flex flex-col justify-between overflow-hidden transition-all duration-500 ${
                    isActive 
                      ? 'bg-white border-slate-200/80 shadow-[0_20px_50px_rgba(0,0,0,0.03)] scale-[1.01]' 
                      : 'bg-white/40 border-slate-100/60 opacity-40 scale-[0.98] blur-[0.4px]'
                  }`}
                >
                  {/* Micro sci-fi branding info */}
                  <span className="absolute top-3 right-5 font-mono text-[0.48rem] tracking-widest text-slate-400 uppercase">
                    VISUALIZATION // {ind.id.toUpperCase()}
                  </span>

                  <div className="grid sm:grid-cols-[1.1fr_1.3fr] gap-6 items-center">
                    
                    {/* Left part: Clean header with icon */}
                    <div>
                      <div className="flex items-center gap-3.5 mb-2">
                        <div
                          className="w-10 h-10 rounded-xl flex items-center justify-center shrink-0 border transition-all duration-300"
                          style={{
                            background: isActive ? ind.accent + '15' : '#f8fafc',
                            borderColor: isActive ? ind.accent + '35' : '#e2e8f0',
                            color: isActive ? ind.accent : '#94a3b8',
                          }}
                        >
                          {ind.icon}
                        </div>
                        <h3 className={`font-display text-[1.15rem] md:text-[1.25rem] font-extrabold tracking-tight transition-colors duration-300 ${
                          isActive ? 'text-slate-900' : 'text-slate-500'
                        }`}>
                          {ind.name}
                        </h3>
                      </div>
                      <p className="font-body text-[0.78rem] text-slate-500 font-medium">
                        {ind.tagline}
                      </p>
                    </div>

                    {/* Right part: Dynamic Visualization */}
                    <div className={`rounded-2xl border flex items-center justify-center p-3 relative overflow-hidden aspect-video transition-all duration-500 ${
                      isActive ? 'bg-slate-50/50 border-slate-100/50' : 'bg-transparent border-transparent'
                    }`}>
                      <div className="absolute inset-0 opacity-[0.01] bg-[radial-gradient(#000_1px,transparent_1px)] [background-size:12px_12px]" />
                      <div className="relative w-full max-w-[170px] h-full flex items-center justify-center">
