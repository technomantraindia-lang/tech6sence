          ...
          <div className="flex flex-wrap gap-3">
            {successStoriesData.map((industry) => (
              <button
                ...
                onClick={() => scrollToIndustry(industry.id)}
                className="px-5 py-2.5 rounded-full bg-white border border-slate-200 text-sm font-semibold text-slate-700 hover:border-violet-300 hover:text-violet-700 hover:shadow-sm transition-all duration-300"
              >
                {industry.title}
              </button>
            ))}
                  ...
                  <div key={story.id} className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-20">
                    ...
                    <div className="lg:col-span-4 relative h-full">
                      <div className="sticky top-32 self-start pb-8 z-10" style={{ position: 'sticky', top: '120px', height: 'max-content' }}>
                        <span className="text-violet-600 font-bold uppercase tracking-wider text-sm mb-4 block">
                          Success Story {idx + 1}
                        </span>
                    ...
                    <div className="lg:col-span-8 space-y-12">
                      ...
                      <div>
                        <h4 className="text-xl font-bold text-slate-900 mb-4 border-l-4 border-violet-400 pl-4">TECH6SENSE Solution</h4>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                          {Object.entries(story.results).map(([category, items]) => (
                            <div key={category} className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
                              <h5 className="font-bold text-slate-900 capitalize mb-4 text-sm tracking-wide text-violet-600">
                                {category} Impact
                              </h5>
                      ...
                      {/* Testimonial */}
                      {story.testimonial && (
                        <div className="relative bg-gradient-to-br from-violet-50 to-fuchsia-50 p-8 md:p-10 rounded-3xl border border-violet-100">
                          <svg className="absolute top-6 left-6 w-12 h-12 text-violet-200 opacity-50" fill="currentColor" viewBox="0 0 32 32">
